window.document.addEventListener('DOMContentLoaded' , function main() { 
   ...
   let allBase  = 
    ...
    for (let i = 0; i < buyButton.length; i++) {
        buyButton[i].addEventListener('click', function (event) {
            ...
            if (click && click.classList.contains('btn')) {
                ...
                // Пересчитываем общую стоимость при добавлении товара в корзину
                cartTotal += parseFloat(allBase[0][i].price.replace(/\s/g, '')); // Преобразуем цену в число и добавляем к общей стоимости
    ...
    function updateTotalCostUI() {
        totalCostElement.textContent = `Общая стоимость: ${cartTotal.toFixed(2)}тг`;
    }
    ...
    function buildCartMessage() {
        ...
        message += `Итого: ${cartTotal.toFixed(2)}тг`;
